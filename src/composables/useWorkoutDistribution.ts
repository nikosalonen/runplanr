 ...
 */
const WORKOUT_DISTRIBUTION_TEMPLATES: Record<
	number,
	WorkoutDistributionTemplate
> = {
	3: {
		trainingDays: 3,
...
export function useWorkoutDistribution() {
	 ...
	 */
	function getDistributionTemplate(
		trainingDaysPerWeek: number,
	): WorkoutDistributionTemplate {
		if (trainingDaysPerWeek < 3 || trainingDaysPerWeek > 7) {
			throw new Error(
				`Invalid training days per week: ${trainingDaysPerWeek}. Must be between 3-7.`,
			);
		}
	 ...
	 */
	function getRaceDistanceScaling(
		raceDistance: RaceDistance,
	): RaceDistanceScaling {
	...
	function calculateWorkoutDistances(
		...
		getDistributionTemplate(trainingDaysPerWeek);
		...
		const distances: Record<WorkoutType, number> = {
			...
			quality:
				Math.round(baseDistance * scaling.qualityDistanceMultiplier * 10) / 10,
			rest: 0,
		};
	...
	function createWeeklyDistribution(
		...
		const workoutSchedule = scheduleWorkouts(
			template,
			config,
			workoutDistances,
			phase,
	...
	function scheduleWorkouts(
		...
		const schedule: (Workout | null)[] = new Array(7).fill(null);
		...
		if (longRunDayIndex !== -1) {
			schedule[longRunDayIndex] = createWorkout(
				"long",
				workoutDistances.long,
				phase,
			);
		}

		// Get available training days (excluding rest days and long run day)
		const availableDays = DAYS_OF_WEEK.map((day, index) => ({ day, index }))
			.filter(
				({ day, index }) =>
					!config.restDays.includes(day) && index !== longRunDayIndex,
			)
			.map(({ index }) => index);

		// Place remaining workouts from template (excluding the long run)
		const remainingWorkouts = template.workoutPattern.filter(
			(workout) => workout !== "long",
		);

		for (
			let i = 0;
			i < remainingWorkouts.length && i < availableDays.length;
			i++
		) {
	 ...
	 */
	function getQualityWorkoutDescription(
		distance: number,
		phase: TrainingPhase,
	): string {
		const distanceStr = `${distance} km`;
	 ...
	 */
	function getWorkoutPaceGuidance(
		type: WorkoutType,
		phase: TrainingPhase,
	): string {
		switch (type) {
	 ...
	 */
	function validateWorkoutDistribution(
		distribution: WeeklyWorkoutDistribution,
	): {
		isValid: boolean;
		warnings: string[];
		...
		const easyRunRatio =
			distribution.workoutCounts.easy / distribution.totalWorkouts;
		if (easyRunRatio < 0.6) {
			// At least 60% easy runs for MVP
			warnings.push(
	 ...
	 */
	function getAllRaceDistanceScaling(): Record<
		RaceDistance,
		RaceDistanceScaling
	> {
	...
	function adjustDistributionForPhase(
		...
		adjustedDistribution.dailyWorkouts = baseDistribution.dailyWorkouts.map(
			(day) => {
