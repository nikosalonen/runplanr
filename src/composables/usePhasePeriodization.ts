 ...
 */
const PHASE_DISTRIBUTION_BY_RACE: Record<RaceDistance, {
  base: number;
  build: number;
  peak: number;
  taper: number;
  ...
  '5K': {
    base: 0.30,    // 30% - Less base needed for 5K
    build: 0.40,   // 40% - More speed development
    peak: 0.20,    // 20% - Race sharpening
    taper: 0.10    // 10% - Short taper
  },
  '10K': {
    base: 0.35,    // 35% - Moderate base
    build: 0.35,   // 35% - Balanced build
    peak: 0.20,    // 20% - Race preparation
    taper: 0.10    // 10% - Standard taper
  },
  'Half Marathon': {
    base: 0.40,    // 40% - More aerobic base
    build: 0.30,   // 30% - Threshold development
    peak: 0.20,    // 20% - Race specificity
    taper: 0.10    // 10% - Recovery taper
  },
  'Marathon': {
    base: 0.45,    // 45% - Extensive base building
    build: 0.25,   // 25% - Moderate build
    peak: 0.20,    // 20% - Race preparation
    taper: 0.10    // 10% - Critical taper
  }
};
...
export function usePhasePeriodization() {
  ...
  function calculatePhasePeriodization(
    ...
    const totalMinimumWeeks = Object.values(MINIMUM_PHASE_DURATIONS).reduce((sum, weeks) => sum + weeks, 0);
    ...
    if (totalWeeks < totalMinimumWeeks) {
      ...
      const buildWeeks = Math.max(1, Math.floor(totalWeeks * distribution.build));
      const peakWeeks = Math.max(1, Math.floor(totalWeeks * distribution.peak));
      const taperWeeks = Math.max(1, Math.floor(totalWeeks * distribution.taper));
      
      // Adjust to fit total weeks
      let calculatedTotal = baseWeeks + buildWeeks + peakWeeks + taperWeeks;
      ...
      if (difference !== 0) {
        ...
        const phases = Object.entries(phaseDurations).sort(([,a], [,b]) => b - a);
        ...
        if (phaseDurations[phases[0][0] as TrainingPhase] < 1) {
          ...
          const deficit = 1 - (phaseDurations[phases[0][0] as TrainingPhase] + difference);
          phaseDurations[phases[1][0] as TrainingPhase] = Math.max(1, phaseDurations[phases[1][0] as TrainingPhase] - deficit);
        }
      }
    } else {
      ...
      const totalCalculated = Object.values(phaseDurations).reduce((sum, weeks) => sum + weeks, 0);
      if (totalCalculated !== totalWeeks) {
        ...
        if (phaseDurations.base >= phaseDurations.build) {
          phaseDurations.base = Math.max(MINIMUM_PHASE_DURATIONS.base, phaseDurations.base + adjustment);
        } else {
          phaseDurations.build = Math.max(MINIMUM_PHASE_DURATIONS.build, phaseDurations.build + adjustment);
        }
      }
    }
    ...
    phases.push({
      ...
      workoutEmphasis: ['race pace intervals', 'tune-up races', 'specific workouts']
    });
   ...
   */
  function getPhaseForWeek(weekNumber: number, periodization: PhasePeriodization): TrainingPhase {
   ...
   */
  function getPhaseConfiguration(phase: TrainingPhase, periodization: PhasePeriodization): PhaseConfiguration | undefined {
   ...
   */
  function createPhaseTransitions(phases: PhaseConfiguration[]): PhaseTransition[] {
    ...
    for (let i = 0; i < phases.length - 1; i++) {
      ...
      transitions.push({
        ...
        adjustments: getTransitionAdjustments(currentPhase.phase, nextPhase.phase),
   ...
   */
  function getTransitionAdjustments(fromPhase: TrainingPhase, toPhase: TrainingPhase): string[] {
   ...
   */
  function getTransitionWarnings(fromPhase: TrainingPhase, toPhase: TrainingPhase): string[] {
    const transitionKey = `${fromPhase}-${toPhase}`;
  ...
  function validatePhasePeriodization(periodization: PhasePeriodization): {
    ...
    if (basePhase && basePhase.percentage < 25) {
      warnings.push('Base phase may be too short for adequate aerobic development');
      recommendations.push('Consider extending base phase for better injury prevention');
    }
    ...
    if (periodization.totalWeeks < 8) {
      ...
      recommendations.push('Consider extending program length for better results');
    }
   ...
   */
  function getPhaseRecommendations(raceDistance: RaceDistance, totalWeeks: number): {
    recommendations: string[];
    optimalWeeks: number;
    ...
    switch (raceDistance) {
      case '5K':
        optimalWeeks = 8;
        recommendations.push('5K training benefits from more speed work in build phase');
        if (totalWeeks < 6) {
          phaseAdjustments.push('Consider minimal base phase and focus on speed development');
        }
        break;
        
      case '10K':
        optimalWeeks = 10;
        recommendations.push('10K requires balanced aerobic and anaerobic development');
        if (totalWeeks < 8) {
          phaseAdjustments.push('Compress base phase but maintain build phase duration');
        }
        break;
      ...
      case 'Marathon':
        ...
        if (totalWeeks < 12) {
          ...
          recommendations.push('Consider longer program for optimal marathon preparation');
        }
        break;
    }
