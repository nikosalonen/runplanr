...
const PROGRESSION_CONSTANTS = {
	MAX_WEEKLY_INCREASE: 0.1, // 10% maximum weekly increase
	SAFE_WEEKLY_INCREASE: 0.08, // 8% recommended weekly increase
	DELOAD_REDUCTION_MIN: 0.2, // 20% minimum deload reduction
	DELOAD_REDUCTION_MAX: 0.3, // 30% maximum deload reduction
...
export function useProgressionRules() {
	...
	function calculateWeeklyVolumes(
		...
		const startingDistance = calculateStartingDistance(
			raceDistance,
			userExperience,
		);
		...
		for (let week = 1; week <= programLength; week++) {
			...
			if (isDeloadWeek) {
				...
				const deloadReduction =
					PROGRESSION_CONSTANTS.DELOAD_REDUCTION_MIN +
					Math.random() *
						(PROGRESSION_CONSTANTS.DELOAD_REDUCTION_MAX -
							PROGRESSION_CONSTANTS.DELOAD_REDUCTION_MIN);
				...
				volumes.push({
					...
					notes: [
						`Deload week: ${Math.round(deloadReduction * 100)}% volume reduction for recovery`,
					],
				});
				...
				if (week > 1 && !volumes[week - 2]?.isDeloadWeek) {
					...
					const previousVolume =
						volumes[week - 2]?.adjustedVolume || currentVolume;
					...
					if (weeklyIncrease > PROGRESSION_CONSTANTS.AGGRESSIVE_THRESHOLD) {
						notes.push(
							"Aggressive progression detected - monitor for overtraining signs",
						);
					}
	 ...
	 */
	function calculateDeloadVolume(
		baseVolume: number,
	...
	function validateProgression(
		volumes: WeeklyVolumeCalculation[],
		...
		if (plateauWeeks > totalWeeks * 0.3) {
			adjustments.push(
				"Consider more progressive volume increases for continued adaptation",
			);
		}
	...
	function adjustAggressiveProgression(
		...
		if (proposedIncrease > PROGRESSION_CONSTANTS.MAX_WEEKLY_INCREASE) {
			...
			adjustedVolume =
				previousVolume * (1 + PROGRESSION_CONSTANTS.MAX_WEEKLY_INCREASE);
			notes.push(
		...
		if (adjustedVolume > maxSafeVolume) {
			...
			notes.push(
				`Volume capped at ${maxSafeVolume} km maximum for experience level`,
			);
		}
