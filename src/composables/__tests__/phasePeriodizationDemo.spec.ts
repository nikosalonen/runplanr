...
describe('Phase Periodization Demo', () => {
  const { calculatePhasePeriodization, getPhaseForWeek, getPhaseCharacteristics } = usePhasePeriodization();
  ...
  it('should demonstrate phase periodization for Half Marathon', () => {
    ...
    periodization.phases.forEach(phase => {
      ...
      console.log(`Duration: ${phase.durationWeeks} weeks (${phase.percentage.toFixed(1)}%)`);
  ...
  it('should demonstrate weekly phase progression', () => {
    ...
    for (let week = 1; week <= 16; week++) {
      ...
      console.log(`${week.toString().padStart(4)} | ${phase.padEnd(6)} | ${volume?.adjustedVolume.toString().padStart(4)}km | ${phase} phase${deloadNote}`);
    }
  ...
  it('should demonstrate phase characteristics comparison', () => {
    ...
    const phases: Array<'base' | 'build' | 'peak' | 'taper'> = ['base', 'build', 'peak', 'taper'];
    
    phases.forEach(phase => {
      const characteristics = getPhaseCharacteristics(phase);
      const recommendations = getPhaseWorkoutRecommendations(phase);
      ...
      characteristics.primaryAdaptations.slice(0, 3).forEach(adaptation => 
        console.log(`  • ${adaptation}`)
      );
      console.log(`Key Workout Types: ${characteristics.workoutTypes.join(', ')}`);
      console.log(`Key Metrics: ${characteristics.keyMetrics.slice(0, 2).join(', ')}`);
    });
  ...
  it('should demonstrate race distance adaptations', () => {
    ...
    raceDistances.forEach(distance => {
      ...
      console.log(`${distance.padEnd(13)} | ${basePhase.percentage.toFixed(0).padStart(5)}% | ${buildPhase.percentage.toFixed(0).padStart(6)}% | ${peakPhase.percentage.toFixed(0).padStart(5)}% | ${taperPhase.percentage.toFixed(0).padStart(6)}%`);
    });
  ...
  it('should demonstrate program length adaptations', () => {
    ...
    programLengths.forEach(weeks => {
      ...
      const baseWeeks = periodization.phases.find(p => p.phase === 'base')!.durationWeeks;
      const buildWeeks = periodization.phases.find(p => p.phase === 'build')!.durationWeeks;
      const peakWeeks = periodization.phases.find(p => p.phase === 'peak')!.durationWeeks;
      const taperWeeks = periodization.phases.find(p => p.phase === 'taper')!.durationWeeks;
      
      const notes = weeks < 10 ? 'Compressed phases' : weeks > 16 ? 'Extended base' : 'Optimal';
      
      console.log(`${weeks.toString().padStart(5)} | ${baseWeeks.toString().padStart(4)} | ${buildWeeks.toString().padStart(5)} | ${peakWeeks.toString().padStart(4)} | ${taperWeeks.toString().padStart(5)} | ${notes}`);
    });
