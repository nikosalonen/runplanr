...
const recommendations = progressionRules.getProgressionRecommendations('Half Marathon');
...
console.log(`- Starting distance (intermediate): ${recommendations.startingDistance.intermediate} km`);
console.log(`- Maximum distance (intermediate): ${recommendations.maxDistance.intermediate} km`);
console.log(`- Recommended deload frequency: every ${recommendations.recommendedDeloadFrequency} weeks\n`);
...
weeklyVolumes.forEach(week => {
  ...
  const rate = week.progressionRate > 0 ? `+${Math.round(week.progressionRate * 100)}%` : 
               week.progressionRate < 0 ? `${Math.round(week.progressionRate * 100)}%` : '  0%';
  const notes = week.notes.length > 0 ? week.notes[0].substring(0, 30) + '...' : '';
  
  console.log(`${week.weekNumber.toString().padStart(4)} | ${week.adjustedVolume.toString().padStart(6)} |${deloadIndicator}| ${rate.padStart(4)} | ${notes}`);
});
...
if (validation.adjustments.length > 0) {
  ...
  validation.adjustments.forEach(adjustment => console.log(`- ${adjustment}`));
}

// Example of aggressive progression adjustment
console.log('\n=== Aggressive Progression Example ===');
const aggressiveResult = progressionRules.adjustAggressiveProgression(45, 30, 65);
