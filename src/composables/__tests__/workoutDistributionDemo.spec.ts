...
describe('Workout Distribution Demo', () => {
  const { createWeeklyDistribution, getRaceDistanceScaling, calculateWorkoutDistances } = useWorkoutDistribution();
  ...
  it('should demonstrate basic workout distribution for 10K plan', () => {
    ...
    distribution.dailyWorkouts.forEach(day => {
      if (day.isRestDay) {
        ...
        console.log(`${day.dayOfWeek}: ${day.workout.type.toUpperCase()} - ${day.workout.distance} km`);
      }
    });
  });
  ...
  it('should demonstrate race distance scaling differences', () => {
    ...
    (['5K', '10K', 'Half Marathon', 'Marathon'] as const).forEach(raceDistance => {
      const config = { ...baseConfig, raceDistance };
      const scaling = getRaceDistanceScaling(raceDistance);
      const distances = calculateWorkoutDistances(raceDistance, weeklyVolume, 4);
  ...
  it('should demonstrate metric unit consistency', () => {
    ...
    workouts.forEach(workout => {
      console.log(`${workout.type.toUpperCase()}: ${workout.distance} km (${workout.duration} min)`);
    ...
    console.log(`Average workout distance: ${Math.round(totalDistance / workouts.length)} km`);
  });
});
