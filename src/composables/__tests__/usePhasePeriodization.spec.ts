...
describe('usePhasePeriodization', () => {
  ...
  describe('calculatePhasePeriodization', () => {
    ...
    it('should create proper phase distribution for Marathon', () => {
      ...
      expect(basePhase?.durationWeeks).toBeGreaterThan(buildPhase?.durationWeeks || 0);
    ...
    it('should ensure all weeks are accounted for', () => {
      const totalWeeks = 12;
      const periodization = calculatePhasePeriodization(totalWeeks, 'Half Marathon');
  ...
  describe('getPhaseForWeek', () => {
    ...
    it('should handle edge cases', () => {
      ...
      expect(getPhaseForWeek(buildPhase.startWeek, periodization)).toBe('build');
    });
  });
  ...
  describe('getPhaseConfiguration', () => {
    ...
    it('should return undefined for non-existent phase', () => {
      ...
      const config = getPhaseConfiguration('base' as any, {
        ...periodization,
  ...
  describe('getPhaseCharacteristics', () => {
    it('should return characteristics for all phases', () => {
      ...
      phases.forEach(phase => {
        ...
        expect(characteristics.intensityEmphasis).toMatch(/^(low|moderate|high)$/);
        expect(characteristics.recoveryEmphasis).toMatch(/^(low|moderate|high)$/);
  ...
  describe('validatePhasePeriodization', () => {
    ...
    it('should detect inadequate base phase', () => {
      ...
      expect(validation.warnings.some(w => w.includes('Base phase may be too short'))).toBe(true);
    });
  });

  describe('getPhaseRecommendations', () => {
    it('should provide recommendations for each race distance', () => {
      const raceDistances: RaceDistance[] = ['5K', '10K', 'Half Marathon', 'Marathon'];
    ...
    it('should provide adjustments for short programs', () => {
      ...
      expect(shortMarathon.phaseAdjustments.some(adj => 
        adj.includes('base') || adj.includes('Extend')
      )).toBe(true);
    });
  });
  ...
  describe('phase distribution constants', () => {
    it('should have valid phase distributions', () => {
      Object.entries(PHASE_DISTRIBUTION_BY_RACE).forEach(([distance, distribution]) => {
        const total = distribution.base + distribution.build + distribution.peak + distribution.taper;
  ...
  describe('integration with different program lengths', () => {
    it('should handle various program lengths correctly', () => {
      ...
      programLengths.forEach(weeks => {
        ...
        const totalMinWeeks = Object.values(MINIMUM_PHASE_DURATIONS).reduce((sum, w) => sum + w, 0);
