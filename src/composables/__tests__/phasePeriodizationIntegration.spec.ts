...
describe('Phase Periodization Integration', () => {
  const { calculatePhasePeriodization, getPhaseForWeek } = usePhasePeriodization();
  const { createWeeklyDistribution, adjustDistributionForPhase, getPhaseWorkoutRecommendations } = useWorkoutDistribution();
  ...
  describe('Full Training Plan Integration', () => {
    it('should create a complete periodized training plan', () => {
      ...
      for (let week = 1; week <= mockConfig.programLength; week++) {
        ...
        const phaseAdjustedDistribution = adjustDistributionForPhase(baseDistribution, phase);
      ...
      weeklyPlans.forEach(weekPlan => {
        expect(weekPlan.distribution.totalWorkouts).toBe(mockConfig.trainingDaysPerWeek);
        expect(weekPlan.distribution.workoutCounts.rest).toBe(mockConfig.restDays.length);
    ...
    it('should show different workout characteristics across phases', () => {
      ...
      expect(baseRecommendations.emphasis).toContain('Aerobic enzyme development');
      
      // Build phase should emphasize intensity
      expect(buildRecommendations.intensityGuidance).toContain('high intensity');
      expect(buildRecommendations.emphasis).toContain('Lactate threshold improvement');
    ...
    it('should properly distribute phases across different program lengths', () => {
      ...
      programLengths.forEach(weeks => {
        ...
        expect(taperPhase.durationWeeks).toBeLessThanOrEqual(Math.max(3, Math.floor(weeks * 0.2)));
      });
    });
  ...
  describe('Phase Transition Integration', () => {
    it('should provide smooth transitions between phases', () => {
      ...
      periodization.phaseTransitions.forEach(transition => {
        ...
        const nextPhase = periodization.phases.find(p => p.phase === transition.toPhase)!;
    ...
    it('should provide appropriate guidance for each transition', () => {
      ...
      expect(baseToBuild.adjustments.some(adj => 
        adj.includes('tempo') || adj.includes('intervals')
      )).toBe(true);
      ...
      expect(peakToTaper.adjustments.some(adj => 
  ...
  describe('Workout Distribution Phase Integration', () => {
    it('should adjust workout descriptions based on phase', () => {
      ...
      const buildDistribution = createWeeklyDistribution(mockConfig, 45, 'build');
      
      const baseAdjusted = adjustDistributionForPhase(baseDistribution, 'base');
      const buildAdjusted = adjustDistributionForPhase(buildDistribution, 'build');
    ...
    it('should include phase information in workout notes', () => {
      ...
      const workoutDays = adjusted.dailyWorkouts.filter(day => !day.isRestDay);
  ...
  describe('Volume and Phase Coordination', () => {
    it('should coordinate volume progression with phase characteristics', () => {
      ...
      const taperPhase = periodization.phases.find(p => p.phase === 'taper')!;
      const taperWeeks = volumes.filter(v => 
        v.weekNumber >= taperPhase.startWeek && v.weekNumber <= taperPhase.endWeek
      );
      
      // Taper weeks should generally have lower volume than peak weeks
      const peakPhase = periodization.phases.find(p => p.phase === 'peak')!;
      const peakWeeks = volumes.filter(v => 
        v.weekNumber >= peakPhase.startWeek && v.weekNumber <= peakPhase.endWeek
      );
      
      if (taperWeeks.length > 0 && peakWeeks.length > 0) {
        const avgTaperVolume = taperWeeks.reduce((sum, w) => sum + w.adjustedVolume, 0) / taperWeeks.length;
        const avgPeakVolume = peakWeeks.reduce((sum, w) => sum + w.adjustedVolume, 0) / peakWeeks.length;
