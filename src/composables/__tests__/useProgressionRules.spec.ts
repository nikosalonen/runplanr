...
describe("useProgressionRules", () => {
	...
	describe("calculateStartingDistance", () => {
		it("should return correct starting distance for different race distances and experience levels (in km)", () => {
			// 5K distances
			expect(progressionRules.calculateStartingDistance("5K", "beginner")).toBe(
				24,
			);
			expect(
				progressionRules.calculateStartingDistance("5K", "intermediate"),
			).toBe(32);
			expect(progressionRules.calculateStartingDistance("5K", "advanced")).toBe(
				40,
			);

			// Marathon distances
			expect(
				progressionRules.calculateStartingDistance("Marathon", "beginner"),
			).toBe(48);
			expect(
				progressionRules.calculateStartingDistance("Marathon", "intermediate"),
			).toBe(64);
			expect(
				progressionRules.calculateStartingDistance("Marathon", "advanced"),
			).toBe(80);
		});

		it("should default to intermediate level when experience not specified", () => {
			expect(progressionRules.calculateStartingDistance("Half Marathon")).toBe(
				48,
			);
		});
	});
	...
	describe("getMaxWeeklyDistance", () => {
		it("should return appropriate maximum distance caps (in km)", () => {
			...
			// Marathon caps
			expect(
				progressionRules.getMaxWeeklyDistance("Marathon", "beginner"),
			).toBe(104);
			expect(
				progressionRules.getMaxWeeklyDistance("Marathon", "advanced"),
			).toBe(160);
		});
	});
	...
	describe("calculateDeloadVolume", () => {
		...
		it("should use custom reduction percentage when provided", () => {
			...
			const deloadVolume = progressionRules.calculateDeloadVolume(
				baseVolume,
				customReduction,
			);
	...
	describe("calculateWeeklyVolumes", () => {
		...
		it("should reduce volume during deload weeks", () => {
			...
			if (deloadWeek) {
				...
				const reductionRatio =
					(deloadWeek.baseVolume - deloadWeek.adjustedVolume) /
					deloadWeek.baseVolume;
		...
		it("should not exceed maximum weekly increase of 10%", () => {
			...
			for (let i = 1; i < volumes.length; i++) {
				...
				if (!current.isDeloadWeek && !previous.isDeloadWeek) {
					const increase =
						(current.adjustedVolume - previous.adjustedVolume) /
						previous.adjustedVolume;
					expect(increase).toBeLessThanOrEqual(0.1); // 10% max increase
				}
			}
		});
		...
		it("should not exceed maximum distance caps", () => {
			...
			const maxDistance = progressionRules.getMaxWeeklyDistance(
				"Half Marathon",
				"intermediate",
			);
		...
		it("should handle different experience levels appropriately", () => {
			...
			const beginnerVolumes =
				progressionRules.calculateWeeklyVolumes(beginnerConfig);
			const advancedVolumes =
				progressionRules.calculateWeeklyVolumes(advancedConfig);

			// Advanced should start higher and reach higher peaks
			expect(advancedVolumes[0].adjustedVolume).toBeGreaterThan(
				beginnerVolumes[0].adjustedVolume,
			);
		});
	});
	...
	describe("validateProgression", () => {
		...
		it("should warn about aggressive progression", () => {
			...
			const validation =
				progressionRules.validateProgression(aggressiveVolumes);
		...
		it("should suggest more deload weeks when frequency is too low", () => {
			...
			expect(validation.warnings.some((w) => w.includes("deload weeks"))).toBe(
				true,
			);
		});
	});
	...
	describe("adjustAggressiveProgression", () => {
		...
		it("should cap at maximum safe volume", () => {
			...
			expect(
				result.notes.some((n) => n.includes("maximum for experience level")),
			).toBe(true);
		});
	});
	...
	describe("getProgressionRecommendations", () => {
		it("should return appropriate recommendations for each race distance", () => {
			const marathonRecs =
				progressionRules.getProgressionRecommendations("Marathon");
	...
	describe("integration tests", () => {
		it("should create a complete progression plan that follows all rules", () => {
			...
			for (let i = 1; i < nonDeloadWeeks.length; i++) {
				expect(nonDeloadWeeks[i].adjustedVolume).toBeGreaterThanOrEqual(
					nonDeloadWeeks[i - 1].adjustedVolume,
				);
			}
		});
	});
