...
describe('useWorkoutDistribution', () => {
  ...
  describe('getDistributionTemplate', () => {
    ...
    it('should return correct template for 5 training days', () => {
      ...
      expect(template.workoutPattern).toEqual(['easy', 'quality', 'easy', 'easy', 'long']);
    ...
    it('should throw error for invalid training days', () => {
      expect(() => getDistributionTemplate(2)).toThrow('Invalid training days per week: 2');
      expect(() => getDistributionTemplate(8)).toThrow('Invalid training days per week: 8');
    });
  });
  ...
  describe('getRaceDistanceScaling', () => {
    ...
    it('should have progressive scaling from 5K to Marathon', () => {
      ...
      for (let i = 1; i < scalings.length; i++) {
        expect(scalings[i].baseDistanceMultiplier).toBeGreaterThan(scalings[i-1].baseDistanceMultiplier);
      }
      
      // Weekly volume bases should increase
      for (let i = 1; i < scalings.length; i++) {
        expect(scalings[i].weeklyVolumeBase).toBeGreaterThan(scalings[i-1].weeklyVolumeBase);
      }
    });
  });
  ...
  describe('createWeeklyDistribution', () => {
    ...
    it('should place long run on specified day', () => {
      ...
      const sundayWorkout = distribution.dailyWorkouts.find(day => day.dayOfWeek === 'Sunday');
    ...
    it('should respect rest days', () => {
      ...
      const mondayWorkout = distribution.dailyWorkouts.find(day => day.dayOfWeek === 'Monday');
      const fridayWorkout = distribution.dailyWorkouts.find(day => day.dayOfWeek === 'Friday');
    ...
    it('should handle different training day frequencies', () => {
      ...
      configs.forEach(config => {
        ...
        expect(distribution.workoutCounts.rest).toBe(7 - config.trainingDaysPerWeek);
  ...
  describe('validateWorkoutDistribution', () => {
    ...
    it('should warn about insufficient training frequency', () => {
      ...
      expect(validation.warnings).toContain('Less than 3 training days per week may limit training effectiveness');
    });
    ...
    it('should warn about too many quality workouts', () => {
      ...
      expect(validation.warnings).toContain('More than 2 quality workouts per week may increase injury risk');
      expect(validation.recommendations).toContain('Limit quality workouts to 1-2 per week for optimal recovery');
    });
    ...
    it('should warn about missing long run', () => {
      ...
      expect(validation.isValid).toBe(false);
      expect(validation.warnings).toContain('No long run scheduled - important for endurance development');
    });
    ...
    it('should warn about no rest days', () => {
      ...
      expect(validation.warnings).toContain('No rest days scheduled - recovery is essential for adaptation');
      expect(validation.recommendations).toContain('Include at least 1 rest day per week');
    });
    ...
    it('should warn about insufficient easy running', () => {
      ...
      expect(validation.warnings).toContain('Consider more easy runs to follow the 80/20 training principle');
    });
  });
  ...
  describe('getAllRaceDistanceScaling', () => {
    it('should return all race distance scalings', () => {
      ...
      expect(Object.keys(scalings)).toEqual(['5K', '10K', 'Half Marathon', 'Marathon']);
      
      Object.values(scalings).forEach(scaling => {
        expect(scaling.baseDistanceMultiplier).toBeGreaterThan(0);
        expect(scaling.longRunMultiplier).toBeGreaterThan(scaling.baseDistanceMultiplier);
  ...
  describe('workout type distribution', () => {
    it('should follow mostly easy runs principle', () => {
      ...
      expect(distribution.workoutCounts.easy).toBeGreaterThan(distribution.workoutCounts.quality);
      
      // Should have exactly one long run
      expect(distribution.workoutCounts.long).toBe(1);
      
      // Easy runs should be the majority of training runs
      const trainingRuns = distribution.workoutCounts.easy + distribution.workoutCounts.quality + distribution.workoutCounts.long;
      expect(distribution.workoutCounts.easy / trainingRuns).toBeGreaterThan(0.5);
    });
