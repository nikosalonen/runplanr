...
import type { DailyWorkout, TrainingPhase, WorkoutType } from "@/types/workout";
 ...
 */
export function calculatePlanMetadata(weeks: WeeklyPlan[]): PlanMetadata {
...
export function calculatePlanStatistics(plan: TrainingPlan): PlanStatistics {
	...
	const averageWeeklyMileage =
		weeklyMileages.reduce((sum, miles) => sum + miles, 0) / weeks.length;
...
export function validatePlanStructure(plan: TrainingPlan): string[] {
	...
	if (!plan.weeks || plan.weeks.length === 0)
		errors.push("Plan must have weeks");
	...
	if (plan.weeks.length !== expectedWeeks) {
		errors.push(
			`Expected ${expectedWeeks} weeks, but found ${plan.weeks.length}`,
		);
	}
	...
	for (let i = 0; i < plan.weeks.length; i++) {
		if (plan.weeks[i].weekNumber !== i + 1) {
			errors.push(
				`Week ${i + 1} has incorrect week number: ${plan.weeks[i].weekNumber}`,
			);
		}
	}
	...
	plan.weeks.forEach((week, weekIndex) => {
		if (week.days.length !== 7) {
			errors.push(
				`Week ${weekIndex + 1} must have 7 days, but has ${week.days.length}`,
			);
		}
		...
		if (restDayCount !== expectedRestDays) {
			errors.push(
				`Week ${weekIndex + 1} has ${restDayCount} rest days, expected ${expectedRestDays}`,
			);
		}
	});
...
function compareWeeks(
	originalWeeks: WeeklyPlan[],
	modifiedWeeks: WeeklyPlan[],
): PlanChange[] {
	const changes: PlanChange[] = [];
 ...
 */
function determineConfigurationChangeImpact(
	field: keyof PlanConfiguration,
): "low" | "medium" | "high" {
	const highImpactFields: Array<keyof PlanConfiguration> = [
		"raceDistance",
		"programLength",
		"trainingDaysPerWeek",
	];
	const mediumImpactFields: Array<keyof PlanConfiguration> = [
		"restDays",
		"longRunDay",
		"deloadFrequency",
	];
...
function generateImpactAssessment(changes: PlanChange[]): string[] {
	...
	const mediumImpactChanges = changes.filter(
		(change) => change.impact === "medium",
	);

	if (highImpactChanges.length > 0) {
		assessments.push(
			`${highImpactChanges.length} high-impact changes detected. Plan regeneration recommended.`,
		);
	}

	if (mediumImpactChanges.length > 0) {
		assessments.push(
			`${mediumImpactChanges.length} medium-impact changes detected. Review workout distribution.`,
		);
	}
