...
export function validatePlanConfiguration(
	...
	const programLengthValidation = validateProgramLength(
	...
	const trainingDaysValidation = validateTrainingDaysPerWeek(
	...
	// Validate rest days
	const restDaysValidation = validateRestDays(
		config.restDays,
		config.trainingDaysPerWeek,
	);
	...
	const longRunDayValidation = validateLongRunDay(
		config.longRunDay,
		config.restDays,
	);
	...
	const deloadFrequencyValidation = validateDeloadFrequency(
		config.deloadFrequency,
		config.programLength,
	);
 ...
 */
export function validateRaceDistance(
	raceDistance: RaceDistance,
): ValidationResult {
	const errors: ValidationError[] = [];
	const warnings: ValidationWarning[] = [];

	const validDistances: RaceDistance[] = [
		"5K",
		"10K",
		"Half Marathon",
		"Marathon",
	...
	if (!validDistances.includes(raceDistance)) {
		errors.push({
			field: "raceDistance",
			message:
				"Invalid race distance. Must be 5K, 10K, Half Marathon, or Marathon.",
 ...
 */
export function validateProgramLength(
...
export function validateTrainingDaysPerWeek(
	...
	if (trainingDaysPerWeek === 3) {
		warnings.push({
			field: "trainingDaysPerWeek",
			message:
				"Training only 3 days per week may limit your progress. Consider 4-5 days for better results.",
	...
	if (trainingDaysPerWeek >= 6) {
		warnings.push({
			field: "trainingDaysPerWeek",
			message:
				"Training 6+ days per week increases injury risk. Ensure adequate recovery.",
...
export function validateRestDays(
	...
	if (hasConsecutiveRestDays(restDays)) {
		warnings.push({
			field: "restDays",
			message:
				"Consecutive rest days may disrupt training rhythm. Consider spreading them out.",
 ...
 */
export function validateLongRunDay(
	...
	if (!["Saturday", "Sunday"].includes(longRunDay)) {
		warnings.push({
			field: "longRunDay",
			message:
				"Long runs are typically scheduled on weekends for better recovery.",
 ...
 */
export function validateDeloadFrequency(
	deloadFrequency: 3 | 4,
	programLength: number,
): ValidationResult {
	const errors: ValidationError[] = [];
...
function performCrossValidation(config: PlanConfiguration): ValidationResult {
	...
	if (config.raceDistance === "Marathon" && config.programLength < 12) {
		warnings.push({
			field: "programLength",
			message:
				"Marathon training with less than 12 weeks significantly increases injury risk.",
	...
	if (config.trainingDaysPerWeek >= 6 && config.programLength < 8) {
		warnings.push({
			field: "trainingDaysPerWeek",
			message:
				"High training frequency with short program duration may lead to overtraining.",
	...
	if (config.raceDistance === "5K" && config.programLength > 16) {
		warnings.push({
			field: "programLength",
			message:
				"5K training programs longer than 16 weeks may lead to staleness.",
...
export function createDefaultConfiguration(
	...
	const constraints =
		CONFIGURATION_CONSTRAINTS.programLength.recommended[raceDistance];
 ...
 */
export function suggestConfigurationImprovements(
	config: PlanConfiguration,
): string[] {
	...
	if (config.trainingDaysPerWeek === 3) {
		suggestions.push(
			"Consider increasing to 4-5 training days per week for better fitness gains.",
		);
	}

	if (!["Saturday", "Sunday"].includes(config.longRunDay)) {
		suggestions.push(
			"Schedule long runs on weekends for better recovery and time availability.",
		);
	}

	const raceConstraints =
		CONFIGURATION_CONSTRAINTS.programLength.recommended[config.raceDistance];
	if (config.programLength < raceConstraints.optimal) {
		suggestions.push(
			`Consider extending to ${raceConstraints.optimal} weeks for optimal ${config.raceDistance} preparation.`,
		);
	}
